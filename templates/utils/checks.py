...
def check_accessibility(driver, blocking_impacts: set | None = None) -> None:
    """axe-core scan on current page. Fails on critical/serious violations only."""
...
def check_no_horizontal_overflow(driver, label: str = '') -> None:
    ...
    assert scroll_width <= client_width + 1, \
        f'{label}: horizontal overflow ({scroll_width}px content in {client_width}px viewport)'
...
def check_load_budget(driver, budget_ms: int = LOAD_TIME_BUDGET_MS) -> None:
    ...
    assert load_ms < budget_ms, f'load time {load_ms}ms exceeds {budget_ms}ms budget'
...
def check_no_oversized_images(driver, max_bytes: int = MAX_IMAGE_BYTES) -> None:
    ...
    assert not offenders, 'oversized images (>{}KB): {}'.format(
        max_bytes // 1024, ', '.join(f"{o['name']} ({o['size'] // 1024}KB)" for o in offenders)
    )


def check_csrf_token(driver) -> None:
    els = driver.find_elements(By.CSS_SELECTOR, 'input[name=csrf_token], input[name=_token]')
    assert els, 'CSRF token missing from form'


def check_required_fields(driver, fields: list[str]) -> None:
    for field in fields:
        assert driver.find_element(By.NAME, field), f'required field "{field}" not found'
